...
const HorizontalPage = () => {
  ...
  const horizontalInstances = trafficLightInstances.filter(tl => tl.orientation === 'horizontal');
    ...
    <div className="flex flex-col items-center w-full max-w-[1400px] mx-auto p-4 sm:p-8">
      ...
      <div className="grid w-full grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 sm:gap-12 justify-items-center">
        {isLoading ? (
          ...
          horizontalInstances.map((instance, index) => (
            <div key={instance.id} className="w-full max-w-[320px] flex flex-col items-center gap-4 group p-5 rounded-2xl border border-white/10 bg-base-200/40">
              <div className="flex items-center gap-2 opacity-40 group-hover:opacity-100 transition-opacity">
                <div className="h-[1px] w-4 bg-white/20" />
                <span className="text-[10px] font-black uppercase tracking-[0.2em]">Instance {index + 1}</span>
                <div className="h-[1px] w-4 bg-white/20" />
              </div>
              <TrafficLights orientation="horizontal" instanceId={instance.id} />
            </div>
          ))
        )}
      </div>
